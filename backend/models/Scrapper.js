...
async function getEveryone() {
  try {
    ...
    if (response.status == 200) {
      ...
      let doiNumber = doiLink.match(/10\.(\d+)/)[1];
      ...
      // MongoDB'ye kaydet
      await saveToMongoDB(title, writer, doiNumber, abstract, section, keywords, article_keywords, date, referans, link , citation_count);
    } else {
